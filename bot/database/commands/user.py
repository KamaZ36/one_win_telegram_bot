...
async def get_count_user_reg_in_one_win(session: AsyncSession) -> int:
    query = select(func.count(User.id)).where(User.tg_id == None)
    response = await session.execute(query)
    return response.scalar()


async def get_count_user_reg_tg_one_wim(session: AsyncSession) -> int:
    query = select(func.count(User.id)).where(User.tg_id != None)
    response = await session.execute(query)
    return response.scalar()


async def get_count_user_in_vip_access(session: AsyncSession) -> int:
    query = select(func.count(User.id)).where(User.vip_access == True)
    response = await session.execute(query)
    return response.scalar()


async def get_count_user_in_deposit(session: AsyncSession) -> int:
    query = select(func.count(User.id)).where(User.deposit == True)
    response = await session.execute(query)
    return response.scalar()
